...
import '/totus/hallazgos/edit_impact_type/edit_impact_type_widget.dart';
import 'dart:ui';
...
import 'impact_type_model.dart';
export 'impact_type_model.dart';

class ImpactTypeWidget extends StatefulWidget {
  const ImpactTypeWidget({super.key});
  static String routeName = 'ImpactType';
  static String routePath = '/ImpactType';
  @override
  State<ImpactTypeWidget> createState() => _ImpactTypeWidgetState();
}

class _ImpactTypeWidgetState extends State<ImpactTypeWidget> with WidgetsBindingObserver, InternetCheckMixin {
  late ImpactTypeModel _model;
  final scaffoldKey = GlobalKey<ScaffoldState>();

  @override
  void initState() {
    super.initState();
    _model = createModel(context, () => ImpactTypeModel());
  ...
  Future<bool?> _showDeleteDialog(BuildContext ctx, String name) => showDialog<bool>(
    ...
    builder: (dc) => Dialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0)), child: Padding(padding: const EdgeInsets.all(24.0), child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 16.0), const Text('Eliminar tipo de impacto', style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold)),
  ...
  Widget _buildItem(ImpactType item) {
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(20.0), child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Row(mainAxisSize: MainAxisSize.max, children: [
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: Builder(builder: (context) => InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            await showDialog(context: context, builder: (dc) => Dialog(elevation: 0, insetPadding: EdgeInsets.zero, backgroundColor: Colors.transparent, alignment: AlignmentDirectional(0.0, 0.0).resolve(Directionality.of(context)), child: GestureDetector(onTap: () { FocusScope.of(dc).unfocus(); FocusManager.instance.primaryFocus?.unfocus(); }, child: EditImpactTypeWidget(conexion: _model.estaconectado!, idImpactType: item.impactTypeId, nombre: item.name, update: (list) async { _model.listImpactType = list; safeSetState(() {}); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Tipo de Impacto Actualizado', style: TextStyle(color: FlutterFlowTheme.of(context).secondaryBackground)), duration: const Duration(milliseconds: 4000), backgroundColor: FlutterFlowTheme.of(context).secondary)); }))));
          ...
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            ...
            if (_model.estaconectado ?? false) {
              await SupaFlow.client.from('ImpactTypes').delete().eq('impact_type_id', item.impactTypeId ?? '');
              final data = await SupaFlow.client.from('ImpactTypes').select().eq('status', true).order('name');
              _model.listImpactType = (data as List).map((e) => ImpactType.fromSupabase(e as Map<String, dynamic>)).toList();
            } else {
              await DBImpactType.deleteImpactType(item.impactTypeId ?? '');
              _model.listImpactType = await DBImpactType.getAllImpactTypes();
            }
            safeSetState(() {});
      ...
      child: SingleChildScrollView(child: Padding(padding: const EdgeInsets.fromLTRB(16.0, 8.0, 16.0, 32.0), child: Container(
        ...
        child: Padding(padding: const EdgeInsets.all(16.0), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 4.0, 0.0, 0.0), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Crear Tipo de Impacto', style: FlutterFlowTheme.of(ctx).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle), fontSize: 20.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle)),
          ...
          Form(key: _model.formKey, autovalidateMode: AutovalidateMode.disabled, child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 8.0, 0.0, 0.0), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [Expanded(child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 10.0, 0.0, 10.0), child: FFButtonWidget(
              onPressed: () async {
                ...
                if (nombreIngresado.isEmpty) { mostrarError('Ingresa el nombre del tipo de impacto'); return; }
                if (_model.estaconectado ?? false) {
                  final nuevoId = random_data.randomString(1, 5, true, false, true);
                  await SupaFlow.client.from('ImpactTypes').insert({'impact_type_id': nuevoId, 'name': nombreIngresado, 'status': true, 'created_at': DateTime.now().toIso8601String(), 'updated_at': DateTime.now().toIso8601String()});
                  await DBImpactType.insertImpactType(ImpactType(impactTypeId: nuevoId, name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true), fromSupabase: true);
                  final data = await SupaFlow.client.from('ImpactTypes').select().eq('status', true).order('name');
                  _model.listImpactType = (data as List).map((e) => ImpactType.fromSupabase(e as Map<String, dynamic>)).toList();
                } else {
                  await DBImpactType.insertImpactType(ImpactType(impactTypeId: random_data.randomString(1, 5, true, false, true), name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true));
                  _model.listImpactType = await DBImpactType.getAllImpactTypes();
                }
                safeSetState(() { _model.txtnombreTextController?.clear(); });
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Tipo de Impacto creado exitosamente', style: TextStyle(color: FlutterFlowTheme.of(context).secondaryBackground)), duration: const Duration(milliseconds: 3000), backgroundColor: FlutterFlowTheme.of(context).secondary));
              },
        ...
        body: SafeArea(top: true, child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.max, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Flexible(child: Text('Listado de Tipos de Impacto', maxLines: 2, softWrap: true, style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 18.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle))),
            ...
            Expanded(child: Builder(builder: (context) {
              final list = _model.listImpactType;
