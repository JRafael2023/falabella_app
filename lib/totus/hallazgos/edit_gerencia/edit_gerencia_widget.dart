...
import 'edit_gerencia_model.dart';
export 'edit_gerencia_model.dart';

class EditGerenciaWidget extends StatefulWidget {
  const EditGerenciaWidget({
    super.key,
    bool? conexion,
    this.idGerencia,
    this.nombre,
  ...
  final String? nombre;
  final Future Function(List<GerenciaStruct> listGerencia)? update;

  @override
  State<EditGerenciaWidget> createState() => _EditGerenciaWidgetState();
}
...
class _EditGerenciaWidgetState extends State<EditGerenciaWidget> {
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EditGerenciaModel());
                            ...
                            child: Padding(
                              ...
                              child: FFButtonWidget(
                                onPressed: () async {
                                  ...
                                  if (nombreDuplicado) {
                                    _mostrarError('Ya existe una gerencia con el nombre "$nombreNuevo"');
                                    return;
                                  }

                                  _model.estaconectado = await actions.checkInternetConecction();
                                  if (_model.estaconectado!) {
                                    _model.updateEcosistema = await ManagementsTable().update(
                                      data: {
                                        'name': nombreNuevo,
                                    ...
                                    _model.listGerencias =
                                        _model.returnGerencias!.toList().cast<GerenciaStruct>();
                                    safeSetState(() {});
                                  } else {
                                    await DBGerencia.updateGerencia(
                                      GerenciaStruct(
                                        idGerencia: widget!.idGerencia,
                                        nombre: nombreNuevo,
                                        updateAt: getCurrentTimestamp,
                                        estado: true,
                                      ),
                                    );
                                    _model.gerenciasoffline = await DBGerencia.getAllGerencias();
                                    _model.listGerencias =
                                        _model.gerenciasoffline!.toList().cast<GerenciaStruct>();
                                    safeSetState(() {});
                                  }

                                  Navigator.pop(context);
                                  await widget.update?.call(_model.listGerencias);
                                  safeSetState(() {});
                                },
                                text: 'Guardar',
