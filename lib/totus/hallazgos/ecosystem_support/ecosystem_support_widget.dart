...
import '/totus/hallazgos/edit_ecosystem_support/edit_ecosystem_support_widget.dart';
import 'dart:ui';
...
import 'ecosystem_support_model.dart';
export 'ecosystem_support_model.dart';

class EcosystemSupportWidget extends StatefulWidget {
  const EcosystemSupportWidget({super.key});
  static String routeName = 'EcosystemSupport';
  static String routePath = '/EcosystemSupport';
  @override
  State<EcosystemSupportWidget> createState() => _EcosystemSupportWidgetState();
}

class _EcosystemSupportWidgetState extends State<EcosystemSupportWidget> with WidgetsBindingObserver, InternetCheckMixin {
  late EcosystemSupportModel _model;
  final scaffoldKey = GlobalKey<ScaffoldState>();

  @override
  void initState() {
    super.initState();
    _model = createModel(context, () => EcosystemSupportModel());
  ...
  Future<bool?> _showDeleteDialog(BuildContext ctx, String name) => showDialog<bool>(
    ...
    builder: (dc) => Dialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16.0)), child: Padding(padding: const EdgeInsets.all(24.0), child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 16.0), const Text('Eliminar soporte de ecosistema', style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold)),
  ...
  Widget _buildItem(EcosystemSupport item) {
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(20.0), child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Row(mainAxisSize: MainAxisSize.max, children: [
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: Builder(builder: (context) => InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            await showDialog(context: context, builder: (dc) => Dialog(elevation: 0, insetPadding: EdgeInsets.zero, backgroundColor: Colors.transparent, alignment: AlignmentDirectional(0.0, 0.0).resolve(Directionality.of(context)), child: GestureDetector(onTap: () { FocusScope.of(dc).unfocus(); FocusManager.instance.primaryFocus?.unfocus(); }, child: EditEcosystemSupportWidget(conexion: _model.estaconectado!, idEcosystemSupport: item.ecosystemSupportId, nombre: item.name, update: (list) async { _model.listEcosystemSupport = list; safeSetState(() {}); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Soporte de Ecosistema Actualizado', style: TextStyle(color: FlutterFlowTheme.of(context).secondaryBackground)), duration: const Duration(milliseconds: 4000), backgroundColor: FlutterFlowTheme.of(context).secondary)); }))));
          ...
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            ...
            if (_model.estaconectado ?? false) {
              await SupaFlow.client.from('EcosystemSupports').delete().eq('ecosystem_support_id', item.ecosystemSupportId ?? '');
              final data = await SupaFlow.client.from('EcosystemSupports').select().eq('status', true).order('name');
              _model.listEcosystemSupport = (data as List).map((e) => EcosystemSupport.fromSupabase(e as Map<String, dynamic>)).toList();
            } else {
              await DBEcosystemSupport.deleteEcosystemSupport(item.ecosystemSupportId ?? '');
              _model.listEcosystemSupport = await DBEcosystemSupport.getAllEcosystemSupports();
            }
            safeSetState(() {});
      ...
      child: SingleChildScrollView(child: Padding(padding: const EdgeInsets.fromLTRB(16.0, 8.0, 16.0, 32.0), child: Container(
        ...
        child: Padding(padding: const EdgeInsets.all(16.0), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 4.0, 0.0, 0.0), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Crear Soporte de Ecosistema', style: FlutterFlowTheme.of(ctx).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle), fontSize: 20.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle)),
          ...
          Form(key: _model.formKey, autovalidateMode: AutovalidateMode.disabled, child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 8.0, 0.0, 0.0), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [Expanded(child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 10.0, 0.0, 10.0), child: FFButtonWidget(
              onPressed: () async {
                ...
                if (nombreIngresado.isEmpty) { mostrarError('Ingresa el nombre del soporte de ecosistema'); return; }
                if (_model.estaconectado ?? false) {
                  final nuevoId = random_data.randomString(1, 5, true, false, true);
                  await SupaFlow.client.from('EcosystemSupports').insert({'ecosystem_support_id': nuevoId, 'name': nombreIngresado, 'status': true, 'created_at': DateTime.now().toIso8601String(), 'updated_at': DateTime.now().toIso8601String()});
                  await DBEcosystemSupport.insertEcosystemSupport(EcosystemSupport(ecosystemSupportId: nuevoId, name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true), fromSupabase: true);
                  final data = await SupaFlow.client.from('EcosystemSupports').select().eq('status', true).order('name');
                  _model.listEcosystemSupport = (data as List).map((e) => EcosystemSupport.fromSupabase(e as Map<String, dynamic>)).toList();
                } else {
                  await DBEcosystemSupport.insertEcosystemSupport(EcosystemSupport(ecosystemSupportId: random_data.randomString(1, 5, true, false, true), name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true));
                  _model.listEcosystemSupport = await DBEcosystemSupport.getAllEcosystemSupports();
                }
                safeSetState(() { _model.txtnombreTextController?.clear(); });
                Navigator.pop(ctx);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Soporte de Ecosistema creado exitosamente', style: TextStyle(color: FlutterFlowTheme.of(context).secondaryBackground)), duration: const Duration(milliseconds: 3000), backgroundColor: FlutterFlowTheme.of(context).secondary));
              },
        ...
        body: SafeArea(top: true, child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.max, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Flexible(child: Text('Listado de Soportes de Ecosistema', maxLines: 2, softWrap: true, style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 18.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle))),
            ...
            Expanded(child: Builder(builder: (context) {
              final list = _model.listEcosystemSupport;
