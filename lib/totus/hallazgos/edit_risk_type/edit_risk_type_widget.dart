...
import 'edit_risk_type_model.dart';
export 'edit_risk_type_model.dart';

class EditRiskTypeWidget extends StatefulWidget {
  const EditRiskTypeWidget({
    super.key,
    bool? conexion,
    this.idRiskType,
    this.nombre,
  ...
  final bool conexion;
  final String? idRiskType;
  final String? nombre;
  final Future Function(List<RiskType> listRiskType)? update;

  @override
  State<EditRiskTypeWidget> createState() => _EditRiskTypeWidgetState();
}

class _EditRiskTypeWidgetState extends State<EditRiskTypeWidget> {
  late EditRiskTypeModel _model;
  String? _errorMsg;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EditRiskTypeModel());
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 12.0, 0.0, 0.0),
            child: Text('Editar Tipo de Riesgo', style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 24.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle))),
                  ...
                  Row(children: [Expanded(child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 10.0, 0.0, 10.0),
                    child: FFButtonWidget(
                      onPressed: () async {
                        ...
                        if (nombreNuevo.isEmpty) { _mostrarError('Ingresa el nombre del tipo de riesgo'); return; }
                        _model.estaconectado = await actions.checkInternetConecction();
                        if (_model.estaconectado!) {
                          await SupaFlow.client.from('RiskTypes').update({'name': nombreNuevo, 'updated_at': DateTime.now().toIso8601String()}).eq('risk_type_id', widget.idRiskType ?? '');
                          ...
                          await DBRiskType.updateRiskType(RiskType(riskTypeId: widget.idRiskType, name: nombreNuevo, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true));
                          _model.listRiskType = await DBRiskType.getAllRiskTypes();
                          safeSetState(() {});
                        }
                        Navigator.pop(context);
                        await widget.update?.call(_model.listRiskType);
                        safeSetState(() {});
                      },
                      text: 'Guardar',
