...
import '/totus/hallazgos/edit_risk_typology/edit_risk_typology_widget.dart';
import 'dart:ui';
...
import 'risk_typology_model.dart';
export 'risk_typology_model.dart';

class RiskTypologyWidget extends StatefulWidget {
  const RiskTypologyWidget({super.key});
  static String routeName = 'RiskTypology';
  static String routePath = '/RiskTypology';
  @override
  State<RiskTypologyWidget> createState() => _RiskTypologyWidgetState();
}

class _RiskTypologyWidgetState extends State<RiskTypologyWidget> with WidgetsBindingObserver, InternetCheckMixin {
  late RiskTypologyModel _model;
  final scaffoldKey = GlobalKey<ScaffoldState>();

  @override
  void initState() {
    super.initState();
    _model = createModel(context, () => RiskTypologyModel());
      ...
      await Future.wait([
        ...
        Future(() async {
          ...
          if (_model.conexion!) {
            ...
            final rtData = await SupaFlow.client.from('RiskTypes').select().eq('status', true).order('name');
            _model.listRiskTypes = (rtData as List).map((e) => RiskType.fromSupabase(e as Map<String, dynamic>)).toList();
            safeSetState(() {});
          } else {
  ...
  String _getRiskTypeName(String? id) {
    if (id == null) return '';
    final rt = _model.listRiskTypes.where((t) => t.riskTypeId == id).toList();
    return rt.isNotEmpty ? (rt.first.name ?? '') : id;
  }
  ...
  Widget _buildItem(RiskTypology item) {
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(20.0), child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Expanded(child: Column(mainAxisSize: MainAxisSize.max, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Tipo: ${_getRiskTypeName(item.riskTypeId)}', style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.w200, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 12.0, letterSpacing: 0.0, fontWeight: FontWeight.w200, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle)),
        ...
        Row(mainAxisSize: MainAxisSize.max, children: [
          ...
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            ...
            if (_model.estaconectado ?? false) {
              await SupaFlow.client.from('RiskTypologies').delete().eq('risk_typology_id', item.riskTypologyId ?? '');
              final data = await SupaFlow.client.from('RiskTypologies').select().eq('status', true).order('name');
              _model.listRiskTypology = (data as List).map((e) => RiskTypology.fromSupabase(e as Map<String, dynamic>)).toList();
            } else {
              await DBRiskTypology.deleteRiskTypology(item.riskTypologyId ?? '');
              _model.listRiskTypology = await DBRiskTypology.getAllRiskTypologies();
            }
            safeSetState(() {});
      ...
      child: SingleChildScrollView(child: Padding(padding: const EdgeInsets.fromLTRB(16.0, 8.0, 16.0, 32.0), child: Container(
        ...
        child: Padding(padding: const EdgeInsets.all(16.0), child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Form(key: _model.formKey, autovalidateMode: AutovalidateMode.disabled, child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 8.0, 0.0, 0.0), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Tipo de Riesgo', style: FlutterFlowTheme.of(ctx).bodyMedium.override(font: TextStyle(fontWeight: FlutterFlowTheme.of(ctx).bodyMedium.fontWeight, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle), fontSize: 18.0, letterSpacing: 0.0, fontWeight: FlutterFlowTheme.of(ctx).bodyMedium.fontWeight, fontStyle: FlutterFlowTheme.of(ctx).bodyMedium.fontStyle)),
              StatefulBuilder(builder: (ctx2, setDropState) => DropdownButtonFormField<String>(
                ...
                onChanged: (val) => setDropState(() => _model.selectedRiskTypeId = val),
              )),
            ].divide(const SizedBox(height: 5.0))),
            ...
            Row(children: [Expanded(child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 10.0, 0.0, 10.0), child: FFButtonWidget(
              onPressed: () async {
                ...
                if (_model.estaconectado ?? false) {
                  ...
                  await DBRiskTypology.insertRiskTypology(RiskTypology(riskTypologyId: nuevoId, name: nombreIngresado, riskTypeId: _model.selectedRiskTypeId, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true), fromSupabase: true);
                  final data = await SupaFlow.client.from('RiskTypologies').select().eq('status', true).order('name');
                  _model.listRiskTypology = (data as List).map((e) => RiskTypology.fromSupabase(e as Map<String, dynamic>)).toList();
                } else {
                  await DBRiskTypology.insertRiskTypology(RiskTypology(riskTypologyId: random_data.randomString(1, 5, true, false, true), name: nombreIngresado, riskTypeId: _model.selectedRiskTypeId, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true));
                  _model.listRiskTypology = await DBRiskTypology.getAllRiskTypologies();
                }
        ...
        body: SafeArea(top: true, child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.max, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Flexible(child: Text('Listado de Tipologias de Riesgo', maxLines: 2, softWrap: true, style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 18.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle))),
            ...
            Expanded(child: Builder(builder: (context) {
              final list = _model.listRiskTypology;
