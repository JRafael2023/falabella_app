...
import '/totus/hallazgos/edit_publication_status/edit_publication_status_widget.dart';
import 'dart:ui';
...
import 'publication_status_model.dart';
export 'publication_status_model.dart';

class PublicationStatusWidget extends StatefulWidget {
  const PublicationStatusWidget({super.key});
  static String routeName = 'PublicationStatus';
  static String routePath = '/PublicationStatus';
  @override
  State<PublicationStatusWidget> createState() => _PublicationStatusWidgetState();
}

class _PublicationStatusWidgetState extends State<PublicationStatusWidget> with WidgetsBindingObserver, InternetCheckMixin {
  late PublicationStatusModel _model;
  final scaffoldKey = GlobalKey<ScaffoldState>();

  @override
  void initState() {
    super.initState();
    _model = createModel(context, () => PublicationStatusModel());
  ...
  Widget _buildItem(PublicationStatus item) {
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(20.0), child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Row(mainAxisSize: MainAxisSize.max, children: [
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: Builder(builder: (context) => InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            await showDialog(context: context, builder: (dc) => Dialog(elevation: 0, insetPadding: EdgeInsets.zero, backgroundColor: Colors.transparent, alignment: AlignmentDirectional(0.0, 0.0).resolve(Directionality.of(context)), child: GestureDetector(onTap: () { FocusScope.of(dc).unfocus(); FocusManager.instance.primaryFocus?.unfocus(); }, child: EditPublicationStatusWidget(conexion: _model.estaconectado!, idPublicationStatus: item.publicationStatusId, nombre: item.name, update: (list) async { _model.listPublicationStatus = list; safeSetState(() {}); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Estado de Publicacion Actualizado', style: TextStyle(color: FlutterFlowTheme.of(context).secondaryBackground)), duration: const Duration(milliseconds: 4000), backgroundColor: FlutterFlowTheme.of(context).secondary)); }))));
          ...
          Align(alignment: AlignmentDirectional(0.0, 0.0), child: InkWell(splashColor: Colors.transparent, focusColor: Colors.transparent, hoverColor: Colors.transparent, highlightColor: Colors.transparent, onTap: () async {
            ...
            if (_model.estaconectado ?? false) {
              await SupaFlow.client.from('PublicationStatuses').delete().eq('publication_status_id', item.publicationStatusId ?? '');
              final data = await SupaFlow.client.from('PublicationStatuses').select().eq('status', true).order('name');
              _model.listPublicationStatus = (data as List).map((e) => PublicationStatus.fromSupabase(e as Map<String, dynamic>)).toList();
            } else {
              await DBPublicationStatus.deletePublicationStatus(item.publicationStatusId ?? '');
              _model.listPublicationStatus = await DBPublicationStatus.getAllPublicationStatuses();
            }
            safeSetState(() {});
            ...
            Row(children: [Expanded(child: Padding(padding: const EdgeInsetsDirectional.fromSTEB(0.0, 10.0, 0.0, 10.0), child: FFButtonWidget(
              onPressed: () async {
                ...
                if (_model.estaconectado ?? false) {
                  ...
                  await DBPublicationStatus.insertPublicationStatus(PublicationStatus(publicationStatusId: nuevoId, name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true), fromSupabase: true);
                  final data = await SupaFlow.client.from('PublicationStatuses').select().eq('status', true).order('name');
                  _model.listPublicationStatus = (data as List).map((e) => PublicationStatus.fromSupabase(e as Map<String, dynamic>)).toList();
                } else {
                  await DBPublicationStatus.insertPublicationStatus(PublicationStatus(publicationStatusId: random_data.randomString(1, 5, true, false, true), name: nombreIngresado, createdAt: DateTime.now().toIso8601String(), updatedAt: DateTime.now().toIso8601String(), status: true));
                  _model.listPublicationStatus = await DBPublicationStatus.getAllPublicationStatuses();
                }
        ...
        body: SafeArea(top: true, child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.max, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Flexible(child: Text('Listado de Estados de Publicacion', maxLines: 2, softWrap: true, style: FlutterFlowTheme.of(context).bodyMedium.override(font: TextStyle(fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle), fontSize: 18.0, letterSpacing: 0.0, fontWeight: FontWeight.bold, fontStyle: FlutterFlowTheme.of(context).bodyMedium.fontStyle))),
            ...
            Expanded(child: Builder(builder: (context) {
              final list = _model.listPublicationStatus;
